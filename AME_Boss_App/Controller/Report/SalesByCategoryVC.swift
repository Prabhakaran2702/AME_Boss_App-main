...
extension SalesByCategoryVC : UITextFieldDelegate, BranchProtocol {
...
class SalesByCategoryVC: UIViewController {
    ...
    @IBOutlet weak var categoryName: MetrialTextfield!
    ...
    @objc func openPieChart(){
        ...
        vc.titleString = "Sales By Category"
        ...
        Service.sharedInstance.webServiceInitialGETCall(url: URL, isShowLoader: true, paramValues: dictParams as NSDictionary, headerValues: defaultHeader) { (error, JsonResponse) in
            ...
            if self.departments.count > 0 {
                DispatchQueue.main.async {
                    self.categoryName.text = self.departments[0].Dept_Name
    ...
    func API() {
        ...
        let BusinessCenter = defaults.string(forKey: "BusinessCenter")!
        let dictParams = ["": ""]
        var URL = "SalesByCategoryMobile.php?Branch=" + BusinessCenter + "&FromDate=" + SelectedFromDate + "&ToDate=" + SelectedToDate + "&Department=" + self.categoryName.text!
                                    ...
                                    if let array2 = dictionary as? NSArray{
                                        for obj2 in array2 {
                                            if let dict2  = obj2 as? NSDictionary {
                                                let DEPT_NAME = dict2.value(forKey: "CAT_NAME") as! String
...
extension SalesByCategoryVC : UITableViewDelegate, UITableViewDataSource {
    ...
    func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        if indexPath.section == 0 {
            ...
            cell.deffTitleLabel.text = "Category"
...
class Departments {
    ...
    init(Dept_ID : String, Dept_Name : String){
        self.Dept_ID = Dept_ID
        self.Dept_Name = Dept_Name
    }
    
    func getDept_IDInteger() -> Int {
        return Int(Dept_ID) ?? 0
    }
}
