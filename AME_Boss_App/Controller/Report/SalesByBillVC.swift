...
//  AME_Boss_App
//
//  Created by MOHAMMED ABDUL BASITHK on 18/02/22.
...
extension SalesByBillVC : UITextFieldDelegate {
...
class SalesByBillVC: UIViewController, UITableViewDelegate, UITableViewDataSource {
    ...
    func tableView(_ tableView: UITableView, heightForRowAt indexPath: IndexPath) -> CGFloat {
        if indexPath.section == 0 {
            ...
            return CGFloat(75 + self.singleObj!.array.count * 25)
        }else{
    ...
    func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        if indexPath.section == 0 {
            ...
            cell.animateButton()
            cell.piechartButton.isHidden = true
            return cell
        }else if indexPath.section == 1 {
            ...
            cell.deffTitleLabel.text = ""
            cell.deffTitleLabel2.text = ""
            cell.deffTitleLabel3.text = ""
    ...
    func APIs() {
        ...
        let BusinessCenter = defaults.string(forKey: "BusinessCenter")!
        let dictParams = ["": ""]
        let URL = "SalesByBillMobile.php?Branch=" + BusinessCenter + "&FromDate=" + SelectedFromDate + "&ToDate=" + SelectedToDate
        print(URL)
        Service.sharedInstance.webServiceInitialGETCall(url: URL, isShowLoader: true, paramValues: dictParams as NSDictionary, headerValues: defaultHeader) { (error, JsonResponse) in
            ...
            if status == "1" {
                if let header = JsonResponse!["Data"] as? NSArray {
                    let content = header
                                    ...
                                    if let array2 = sale_date_child as? NSArray{
                                        for obj2 in array2 {
                                            if let dict2  = obj2 as? NSDictionary {
                                                let BILL_NO = dict2.value(forKey: "BILL_NO") as! String
                                                ...
                                                let SingleItemObj = SingleItem(mode: BILL_NO, sst: SST, amount: WITHSST)
