...
extension SalesByDateReportVC : UITextFieldDelegate {
...
class SalesByDateReportVC: UIViewController , UITableViewDelegate, UITableViewDataSource {
    ...
    func APIs() {
        ...
        let BusinessCenter = defaults.string(forKey: "BusinessCenter")!
        let dictParams = ["": ""]
        let URL = "SalesByDate.php?Branch=" + BusinessCenter + "&FromDate=" + SelectedFromDate + "&ToDate=" + SelectedToDate
        print(URL)
                    ...
                    do {
                                    ...
                                    if let array2 = dictionary as? NSArray{
                                        for obj2 in array2 {
                                            if let dict2  = obj2 as? NSDictionary {
                                                let PAYMENT_MODE = dict2.value(forKey: "PAYMENT_MODE") as! String
                                                ...
                                                let SingleItemObj = SingleItem(mode: PAYMENT_MODE, sst: (SST).get2Decimal(), amount: (WITHSST).get2Decimal())
                        ...
                        if let array = Content_jsonResult as? NSArray {
                            for obj in array {
                                ...
                                if let dict = obj as? NSDictionary {
                                    // Header
                                    TotalDayObj.paymentDate = dict.value(forKey: "PaymentDateHeader") as? String ?? ""
                                    //  Child
...
class SingleItem {
    ...
    init(mode:String, sst:String, amount:String){
        self.mode = mode
        self.sst = sst
        self.amount = amount
    }
...
extension String {
    func get2Decimal() -> String {
        let myDouble : Double =  Double(self) ?? 0.0
        return String(format:"%01.2f",myDouble)
    }
}  
