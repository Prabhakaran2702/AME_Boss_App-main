//
//  SalesByItemBillVC.swift
//  AME_Boss_App
//
//  Created by MOHAMMED ABDUL BASITHK on 20/02/22.
...
extension SalesByItemBillVC : UITextFieldDelegate, MenuSelectionProtocol{
...
class SalesByItemBillVC: UIViewController {
    
    @IBOutlet weak var category3: MetrialTextfield!
    ...
    @objc func openPieChart(){
        ...
        vc.titleString = "Sales By Item Bill"
    ...
    @IBAction func category3Clicked(_ sender: Any) {
        let vc = self.storyboard?.instantiateViewController(identifier: "MenuListVC") as! MenuListVC
        vc.delegate = self
        let names = self.items.map{ $0.Dept_Name }
        let ids = self.items.map{ $0.Dept_ID }
        vc.list1 = ids
        vc.list2 = names
        vc.type = 3
        self.present(vc, animated: true, completion: nil)
    ...
    func ItemList(){
        ...
        let BusinessCenter = defaults.string(forKey: "BusinessCenter")!
        let dictParams = ["": ""]
        let URL = "ItemList.php?Branch=" + BusinessCenter + "&DeptID=" + self.selectedDepId + "&CatID=" + self.selectedCatId
        Service.sharedInstance.webServiceInitialGETCall(url: URL, isShowLoader: true, paramValues: dictParams as NSDictionary, headerValues: defaultHeader) { (error, JsonResponse) in
            hideHUDForView()
            if JsonResponse == nil {
                CBGlobalMethods.shared.ShowAlert(TitleString: "Alert", MessageString: "Sorry, No Data Available")
            ...
            if let Output = JsonResponse!["Output"] as? NSDictionary{
                if let data = Output["data"] as? NSArray {
                    for item in data {
                        if let obj = item as? NSDictionary {
                            let a = Departments(Dept_ID: obj["Plu_Code"] as! String, Dept_Name: obj["Product_Name"] as! String)
                            self.items.append(a)
                        }
                    }
                }
            ...
            if self.items.count > 0 {
                DispatchQueue.main.async {
                    self.category3.text = self.items[0].Dept_Name
                    self.API()
                }
            }
    ...
    func API() {
        ...
        let BusinessCenter = defaults.string(forKey: "BusinessCenter")!
        let dictParams = ["": ""]
        var URL : String = "SalesByItemBillWiseMobile.php?Branch=" + BusinessCenter + "&FromDate=" + SelectedFromDate + "&ToDate=" + SelectedToDate + "&Department=" + self.category1.text! + "&Category=" + self.category2.text! + "&Item=" + self.category3.text!
...
extension SalesByItemBillVC : UITableViewDelegate, UITableViewDataSource {
